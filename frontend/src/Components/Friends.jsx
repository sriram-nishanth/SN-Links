...
const Friends = ({ searchQuery }) => {
    ...
    const fetchUserProfile = async () => {
      try {
        ...
        if (response.ok) {
          const data = await response.json();
          if (data.success) {
            const followingIds = new Set(data.data.following.map(id => id.toString()));
  ...
  const handleFollowBack = async (id) => {
    try {
      ...
      const response = await fetch(`http://localhost:3000/api/user/${endpoint}/${id}`, {
        method: method,
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
      ...
      if (response.ok && data.success) {
        ...
        setFollowedFriends((prev) => {
          ...
          if (isFollowing) {
            ...
            if (friend) {
              ...
              const existingChatIndex = activeChats.findIndex(chat => chat.id === id);
        ...
        setToast({ message: data.message || `Failed to ${isFollowing ? 'unfollow' : 'follow'} user`, type: 'error' });
      }
    } catch (error) {
      console.error('Error following/unfollowing user:', error);
      setToast({ message: `Error ${followedFriends.has(id) ? 'unfollowing' : 'following'} user`, type: 'error' });
    } finally {
  ...
  return (
    <div className="w-full max-w-md mx-auto space-y-3 sm:space-y-4">
      ...
      <h2 className="text-white font-semibold text-base sm:text-lg mb-2">Recent Activity</h2>

      {loading && (
        <div className="text-gray-400 text-center py-4">Loading followers...</div>
      )}
