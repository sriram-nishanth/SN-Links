...
const Setting = () => {
  ...
  const handleConnectApp = (app) => {
    ...
    setToast({ type: 'success', message: `${app} ${updatedApps[app] ? 'connected' : 'disconnected'} successfully!` });
    setTimeout(() => setToast({ type: '', message: '' }), 3000);
  };
  ...
  const handleConnectNewApp = () => {
    ...
    setToast({ type: 'success', message: `${newAppName} connected successfully!` });
    setTimeout(() => setToast({ type: '', message: '' }), 3000);
    setNewAppName('');
  ...
  const handleChangePassword = () => {
    ...
    if (passwordData.newPassword.length < 6) {
      ...
      setTimeout(() => setToast({ type: '', message: '', type: 'error' }), 3000);
      return;
    }
    // Simulate password change
    localStorage.setItem('passwordChanged', JSON.stringify({ timestamp: new Date().toISOString() }));
  ...
  const uploadImage = async (file, type) => {
    ...
    try {
      ...
      const response = await axios.put(`http://localhost:3000/api/user/profile/${type === 'profile' ? 'image' : 'cover'}`, formData, {
        headers: {
          Authorization: `Bearer ${token}`,
      ...
      if (response.data.success) {
        ...
        const updatedUser = {
          ...user,
          [type === 'profile' ? 'profileImage' : 'backgroundImage']: response.data.data[type === 'profile' ? 'profileImage' : 'backgroundImage']
        };
        ...
        addActivityLog(`${type === 'profile' ? 'Profile' : 'Cover'} image updated`);
        setToast({ type: 'success', message: `${type === 'profile' ? 'Profile' : 'Cover'} image updated successfully!` });
      }
    } catch (error) {
  ...
  return (
    <div key={languageKey} className="bg-gradient-to-r from-zinc-900 to-slate-900 min-h-screen">
      ...
      <div className="flex flex-col lg:flex-row h-[calc(100vh-80px)] px-3 sm:px-4 md:px-6 py-3 sm:py-4 md:py-6 lg:py-8 gap-3 sm:gap-4 md:gap-6 lg:gap-8">
        ...
        <div className="lg:hidden flex justify-between items-center mb-4">
          ...
          >
            {isMobileMenuOpen ? <BsX className="w-5 h-5" /> : <BsList className="w-5 h-5" />}
        ...
        ">
          ...
          {selected === 1 && (
            <section className="space-y-4 sm:space-y-6 md:space-y-8 max-w-2xl">
              <div>
                <h3 className="text-white text-md sm:text-xl lg:text-xl font-semibold mb-3 sm:mb-4 lg:mb-5">{t('settings.profileInformation')}</h3>
                <div className="grid gap-3 sm:gap-5 md:gap-5">
                  {/* Profile Image */}
                  <div className="space-y-2">
                    <span className="text-white text-sm sm:text-base md:text-md font-medium">{t('settings.profileImage')}</span>
                    <div className="flex items-center gap-4">
                      <div className="relative">
                        <img
                          src={profileImagePreview || user?.profileImage || '/default-avatar.png'}
                          alt="Profile"
                  ...
                  {/* Cover Image */}
                  <div className="space-y-2">
                    <span className="text-white text-sm sm:text-base md:text-md font-medium">{t('settings.coverImage')}</span>
                    <div className="space-y-3">
                      <div className="relative">
                        <img
                          src={coverImagePreview || user?.coverImage || '/default-cover.png'}
                          alt="Cover"
                  ...
                  <div className="space-y-2">
                    <label className="flex flex-col">
                      <span className="text-white mb-2 text-sm sm:text-base md:text-md font-medium">{t('settings.fullName')}</span>
                      <input
                        type="text"
                        value={profile.fullName}
                        onChange={(e) => setProfile({ ...profile, fullName: e.target.value })}
                        placeholder={profile.fullName || t('settings.enterFullName')}
                  ...
                  <div className="space-y-2">
                    <label className="flex flex-col">
                      <span className="text-white mb-2 text-sm sm:text-base md:text-md font-medium">{t('settings.username')}</span>
                      <input
                        type="text"
                        value={profile.username}
                        onChange={(e) => setProfile({ ...profile, username: e.target.value })}
                        placeholder={profile.username || t('settings.enterUsername')}
                        className="bg-[#232323] text-white rounded-lg px-4 sm:px-5 py-3 sm:py-4 text-sm sm:text-base md:text-md outline-none focus:ring-2 focus:ring-yellow-400 transition-all min-h-[48px] sm:min-h-[52px]"
                      />
                    </label>
                  </div>
                  <div className="space-y-2">
                    <label className="flex flex-col">
                      <span className="text-white mb-2 text-sm sm:text-base md:text-md font-medium">{t('settings.bio')}</span>
                      <textarea
                        value={profile.bio}
                        onChange={(e) => setProfile({ ...profile, bio: e.target.value })}
                        placeholder={profile.bio || t('settings.tellAboutYourself')}
          ...
          {selected === 2 && (
            <section className="space-y-4 sm:space-y-6 md:space-y-8 max-w-2xl">
              <div>
                <h3 className="text-white text-md sm:text-xl lg:text-xl font-semibold mb-3 sm:mb-4 lg:mb-5">{t('settings.connectedApps')}</h3>
                <div className="bg-[#232323] rounded-lg p-4 sm:p-5 md:p-6 flex flex-col gap-4 sm:gap-5 md:gap-6">
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-white font-semibold text-sm sm:text-base md:text-md">{t('settings.google')}</span>
                    </div>
                    <button
                    ...
                    >
                      {connectedApps.google ? t('settings.disconnect') : t('settings.connect')}
                    </button>
                  </div>
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-white font-semibold text-sm sm:text-base md:text-md">{t('settings.facebook')}</span>
                    </div>
                    <button
                    ...
                    >
                      {connectedApps.facebook ? t('settings.disconnect') : t('settings.connect')}
                    </button>
                  </div>
                </div>
              </div>
              <div className="pt-2 sm:pt-4">
                <h3 className="text-white text-md sm:text-xl lg:text-xl font-semibold mb-3 sm:mb-4 lg:mb-5">{t('settings.addNewApp')}</h3>
                <button
          ...
          {selected === 3 && (
            <section className="space-y-4 sm:space-y-6 md:space-y-8 max-w-lg">
              <div>
                <h3 className="text-white text-md sm:text-xl lg:text-xl font-semibold mb-3 sm:mb-4 lg:mb-5">{t('settings.changePassword')}</h3>
                <div className="grid gap-4 sm:gap-5 md:gap-6">
                  <div className="space-y-2">
                    <label className="flex flex-col">
                      <span className="text-white mb-2 text-sm sm:text-base md:text-md font-medium">{t('settings.newPassword')}</span>
                      <input
                        type="password"
                  ...
                  <div className="space-y-2">
                    <label className="flex flex-col">
                      <span className="text-white mb-2 text-sm sm:text-base md:text-md font-medium">{t('settings.retypePassword')}</span>
                      <input
                        type="password"
          ...
          {selected === 4 && (
            <section className="space-y-4 sm:space-y-6">
              <div>
                <h3 className="text-white text-md sm:text-xl lg:text-xl font-semibold mb-3 sm:mb-4 lg:mb-5">{t('settings.accountLog')}</h3>
                <div className="bg-[#232323] rounded-lg p-4 sm:p-5 md:p-6 text-gray-400 text-sm sm:text-base md:text-md">
                  <div className="space-y-3 sm:space-y-4">
                    {activityLogs.length > 0 ? (
                      activityLogs.map((log, index) => (
                        <div key={log.id} className={`flex items-center justify-between py-2 ${index < activityLogs.length - 1 ? 'border-b border-gray-600/30' : ''}`}>
          ...
          {selected === 5 && (
            <section className="space-y-3 sm:space-y-4 md:space-y-5 max-w-2xl">
              <div>
                <h3 className="text-white text-md sm:text-xl lg:text-xl font-semibold mb-2 sm:mb-3 lg:mb-3">{t('settings.accountPrivacy')}</h3>
                <div className="bg-[#232323] rounded-lg p-3 sm:p-4 md:p-4">
                  <label className="flex items-center gap-3 py-2">
                    <input
                      type="checkbox"
                      checked={privacySettings.privateAccount}
                      onChange={(e) => handlePrivacyChange('privateAccount', e.target.checked)}
                      className="accent-yellow-400 w-4 h-4 sm:w-5 sm:h-5"
                    />
                    <span className="text-white text-sm sm:text-base md:text-md">{t('settings.privateAccount')}</span>
                  </label>
                </div>
              </div>
              <div>
                <h3 className="text-white text-md sm:text-xl lg:text-xl font-semibold mb-2 sm:mb-3 lg:mb-3">{t('settings.accountStatus')}</h3>
                <div className="bg-[#232323] rounded-lg p-3 sm:p-4 md:p-4">
                  <label className="flex items-center gap-3 py-2">
                    <input
                      type="checkbox"
                      checked={privacySettings.showActivityStatus}
                      onChange={(e) => handlePrivacyChange('showActivityStatus', e.target.checked)}
                      className="accent-yellow-400 w-4 h-4 sm:w-5 sm:h-5"
                    />
                    <span className="text-white text-sm sm:text-base md:text-md">{t('settings.showActivityStatus')}</span>
                  </label>
                </div>
              </div>
              <div>
                <h3 className="text-white text-md sm:text-xl lg:text-xl font-semibold mb-2 sm:mb-3 lg:mb-3">{t('settings.twoFactorAuth')}</h3>
                <div className="bg-[#232323] rounded-lg p-3 sm:p-4 md:p-4 space-y-2 sm:space-y-3">
                  <label className="flex items-center gap-3 py-2">
                    <input
                      ...
                      onChange={(e) => handlePrivacyChange('twoFactorAuth', { ...privacySettings.twoFactorAuth, useAuthApp: e.target.checked })}
                      className="accent-yellow-400 w-4 h-4 sm:w-5 sm:h-5"
                    />
                    <span className="text-white text-sm sm:text-base md:text-md">{t('settings.useAuthApp')}</span>
                  </label>
                  <label className="flex items-center gap-3 py-2">
                    <input
                      type="checkbox"
                      checked={privacySettings.twoFactorAuth.useSms}
                      onChange={(e) => handlePrivacyChange('twoFactorAuth', { ...privacySettings.twoFactorAuth, useSms: e.target.checked })}
                      className="accent-yellow-400 w-4 h-4 sm:w-5 sm:h-5"
                    />
                    <span className="text-white text-sm sm:text-base md:text-md">{t('settings.useSms')}</span>
                  </label>
                </div>
              </div>
            </section>
          ...
          {selected === 6 && (
            <section className="space-y-4 sm:space-y-6 md:space-y-8 max-w-2xl">
              <div>
                <h3 className="text-white text-md sm:text-xl lg:text-xl font-semibold mb-3 sm:mb-4 lg:mb-5">{t('settings.otherSettings')}</h3>
                <div className="bg-[#232323] rounded-lg p-4 sm:p-5 md:p-6 space-y-4 sm:space-y-5 md:space-y-6">
                  ...
                  <div className="space-y-2">
                    <label className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
                      <span className="text-white font-semibold text-sm sm:text-base md:text-md min-w-0 sm:min-w-[128px]">{t('settings.language')}</span>
                      <select
                  ...
                  <div className="space-y-2">
                    <label className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
                      <span className="text-white font-semibold text-sm sm:text-base md:text-md min-w-0 sm:min-w-[128px]">{t('settings.notifications')}</span>
                      <div className="flex items-center gap-3 py-2">
                        <input
                          ...
                          className="accent-yellow-400 w-4 h-4 sm:w-5 sm:h-5"
                        />
                        <span className="text-white text-sm sm:text-base md:text-md">{t('settings.enableNotifications')}</span>
                      </div>
                    </label>
                  </div>
                </div>
          ...
          {selected === 7 && (
            <section className="flex flex-col items-center justify-center h-full">
              <h2 className="text-white text-xl sm:text-2xl lg:text-3xl font-semibold mb-4">{t('settings.logOut')}</h2>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-[#181818] rounded-2xl p-6 sm:p-8 max-w-md w-full mx-4">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-white text-xl font-semibold">{t('settings.connectNewApp')}</h3>
              <button
          ...
          <div className="bg-[#181818] rounded-2xl p-6 sm:p-8 max-w-md w-full mx-4">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-white text-xl font-semibold">
                {currentImageType === 'profile' ? t('settings.changeProfileImage') : t('settings.changeCoverImage')}
              </h3>
              <button
            ...
            <div className="space-y-4">
              ...
              <input
                ref={currentImageType === 'profile' ? profileImageRef : coverImageRef}
                type="file"
                accept="image/*"
              ...
              <div className="flex gap-3">
                ...
                <button
                  onClick={() => {
                    const fileInput = currentImageType === 'profile' ? profileImageRef.current : coverImageRef.current;
                    fileInput?.click();
                  }}
                  className="flex-1 bg-yellow-400 text-black font-semibold py-3 rounded-lg hover:bg-yellow-500 transition-all"
                >
      ...
      <footer className="text-gray-400 text-xs text-right pr-6 sm:pr-8 pb-2">Copyright @ {new Date().getFullYear()}</footer>
    </div>
  )
}
