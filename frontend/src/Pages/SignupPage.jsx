...
const SignupPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setToast({ message: err.message || 'Error creating account', type: 'error' });
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="relative p-4 sm:p-8 flex justify-center items-center w-full min-h-screen text-white">
        ...
        <form className="flex flex-col gap-3 sm:gap-4 lg:gap-6" onSubmit={handleSubmit}>
          <div className="flex flex-col">
            <label className="mb-2 text-sm sm:text-base">{t('signup.fullName')}</label>
            <input
              type="text"
              name="fullName"
          ...
          <div className="flex flex-col">
            <label className="mb-2 text-sm sm:text-base">{t('signup.confirmPassword')}</label>
            <input
              type="password"
          ...
          <div className="flex flex-col">
            <label className="mb-2 text-sm sm:text-base">Bio (Optional)</label>
            <textarea
              name="bio"
              value={formData.bio}
              onChange={handleChange}
          ...
          <div className="flex flex-col">
            <label className="mb-2 text-sm sm:text-base">Profile Image (Optional)</label>
            <input
              type="file"
              name="profileImage"
              onChange={(e) => {
          ...
          <div className="flex flex-col text-xs sm:text-sm">
            <label className="flex items-center gap-2">
              <input type="checkbox" className="accent-yellow-400 w-4 h-4" required />
              ...
              <GoogleLogin
                onSuccess={async (credentialResponse) => {
                  try {
                    ...
                    setToast({ message: 'Google authentication successful!', type: 'success' });
                    navigate('/home');
                  } catch (err) {
        ...
        <p className="text-xs text-center text-gray-400 mt-4 sm:mt-6">
          {t('signup.alreadyHaveAccount')} <span className="text-yellow-400 cursor-pointer hover:underline" onClick={() => navigate('/')}>{t('signup.signIn')}</span>
        </p>
