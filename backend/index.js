...
io.on('connection', async (socket) => {
  ...
  try {
    ...
    await User.default.findByIdAndUpdate(socket.userId, { online: true });
  } catch (error) {
  ...
  socket.on('send_message', async (data) => {
    try {
      ...
      const User = await import('./models/user.js');
      const sender = await User.default.findById(socket.userId);
      if (!sender.following.includes(receiverId)) {
        socket.emit('message_error', { error: 'You must follow this user to send messages' });
        return;
      }
  ...
  socket.on('typing_start', (receiverId) => {
    const roomId = [socket.userId, receiverId].sort().join('_');
    socket.to(roomId).emit('user_typing', { userId: socket.userId, isTyping: true });
  });

  socket.on('typing_stop', (receiverId) => {
    const roomId = [socket.userId, receiverId].sort().join('_');
    socket.to(roomId).emit('user_typing', { userId: socket.userId, isTyping: false });
  });
  ...
  socket.on('follow_user', async (data) => {
    try {
      ...
      // Update database
      await User.default.findByIdAndUpdate(socket.userId, {
        $push: { following: targetUserId }
      });
      await User.default.findByIdAndUpdate(targetUserId, {
        $push: { followers: socket.userId }
      });

      // Update online status
      await User.default.findByIdAndUpdate(socket.userId, { online: true });
      await User.default.findByIdAndUpdate(targetUserId, { online: true });
  ...
  socket.on('disconnect', async () => {
    ...
    try {
      const User = await import('./models/user.js');
      await User.default.findByIdAndUpdate(socket.userId, { online: false });
    } catch (error) {
      console.error('Error updating online status:', error);
    }
